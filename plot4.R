...
#Remove all rows containin "?" in Voltage column in subset1 dataset
subset1$Voltage <- as.character(subset1$Voltage)
subset1 <- subset1[subset1$Voltage!="?",]

#Remove all rows containin "?" in Global_reactive_power column in subset1 dataset
subset1$Global_reactive_power <- as.character(subset1$Global_reactive_power)
subset1 <- subset1[subset1$Global_reactive_power!="?",]
...
#Create Frequency Plot for Voltage
subset1$Voltage <- as.numeric(subset1$Voltage)
#subset1$datetimemerge <- as.POSIXct(paste(subset1$Date,as.character(subset1$Time)),format="%Y-%m-%d %H:%M:%S")
#par(bg="WHITE")
plot(subset1$datetimemerge,subset1$Voltage,type="l",xlab="datetime",ylab="Voltage",main="") 
...
#Create Frequency Plot for Global_reactive_power
subset1$Global_reactive_power <- as.numeric(subset1$Global_reactive_power)
#subset1$datetimemerge <- as.POSIXct(paste(subset1$Date,as.character(subset1$Time)),format="%Y-%m-%d %H:%M:%S")
#par(bg="WHITE")
plot(subset1$datetimemerge,subset1$Global_reactive_power,type="l",xlab="datetime",ylab="Global_reactive_power",main="") 
